...
async fn components_demo() -> impl IntoResponse {
    let component = components_page();
    Html(rusti::render_to_string(&component))
}

async fn todo_demo() -> impl IntoResponse {
    let component = todo_page();
    Html(rusti::render_to_string(&component))
}
...
async fn main() {
    let app = Router::new()
        ...
        .route("/components", get(components_demo))
        .route("/todo", get(todo_demo))
    ...
    println!("   http://127.0.0.1:3000/components");
    println!("   http://127.0.0.1:3000/todo");
