...
pub fn interactive_counter() -> impl rusti::Component {
    rusti! {
        ...
        <head>
            ...
            <style>
                body { font-family: sans-serif; max-width: 800px; margin: 50px auto; padding: 20px; min-height: 100vh; background: skyblue; }
                .container { background: white; border-radius: 12px; padding: 30px; }
                h1 { text-align: center; margin-bottom: 30px; }
                .counter-display { text-align: center; margin: 30px 0; }
                .counter-value { font-size: 72px; font-weight: bold; }
                .status { font-size: 20px; margin-top: 10px; }
                .controls { display: flex; gap: 10px; justify-content: center; margin: 20px 0; }
                button { padding: 12px 24px; font-size: 16px; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; }
                button:hover { transform: translateY(-2px); }
                .btn-increment { background: lightgreen; }
                .btn-decrement { background: lightcoral; }
                .btn-reset { background: lightblue; }
                .history { margin-top: 30px; padding: 20px; background: whitesmoke; border-radius: 8px; }
                .history h3 { margin-top: 0; }
                .history-list { list-style: none; padding: 0; }
                .history-item { padding: 8px 12px; margin: 5px 0; background: white; border-radius: 4px; }
            </style>
        </head>
        <body>
            <div class="container">
