...
### 4. Components (Mandatory `#[component]`)

**All components MUST use `#[azumi::component]` macro:**

```rust
#[azumi::component]
fn button<'a>(text: &'a str, variant: &'a str, disabled: bool) -> impl azumi::Component + 'a {
    html! {
        <style src="/static/button.css" />
        <button class={format!("btn-{}", variant)} disabled={disabled}>
            {text}
        </button>
    }
}

// ✅ Correct: Named arguments with @ syntax
@button(text="Click Me", variant="primary", disabled=false)

// ❌ Error: Positional arguments not allowed
button("Click", "primary", false)  // Won't compile!
```

**Features:**

-   **Mandatory usage** - `@ComponentName(...)` requires `#[component]`
-   **Named arguments only** - `@button(text="...", variant="...")`
-   **Lifetime support** - `<'a>` parameters work seamlessly
-   **Type-safe builders** - Auto-generated from params
-   **Children support** - Use `children: impl Component` param

**Note:** The `#[component]` macro generates a builder pattern, making named arguments mandatory. This ensures consistency and prevents positional argument bugs.
